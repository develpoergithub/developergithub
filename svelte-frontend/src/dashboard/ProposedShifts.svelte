<script>
  ...
  let heads = ["Requesting For", "By", "Requesting Shift", "On"];
  let bodies = [];
  ...
  $: if ($shiftConnections.length > 0) {
    bodies = [];
    var proposedShifts = $shiftConnections.filter(
      x => x.proposedShift.postedBy.id === $user.id
    );
    proposedShifts.forEach(element => {
      ...
      var pSPostedBy =
        element.shift.postedBy.userprofile.firstName +
        " " +
        element.shift.postedBy.userprofile.lastName;
...
<main in:fade={{ duration: 500 }}>
  <div>
    ...
    {:else}
      <h5>You have not proposed any shift!</h5>
    {/if}
  </div>
</main>
